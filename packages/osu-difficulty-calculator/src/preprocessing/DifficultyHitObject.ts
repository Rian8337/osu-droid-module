...
export abstract class DifficultyHitObject {
    ...
    protected get maximumSliderRadius(): number {
        return this.normalizedRadius * 2.4;
    }

    protected get assumedSliderRadius(): number {
        return this.normalizedRadius * 1.8;
    }
    ...
    opacityAt(time: number, mods?: ModMap): number {
        ...
        // Equal to `HitObject.timeFadeIn` without adjustments from the Hidden mod itself.
        const fadeInDuration =
            400 * Math.min(1, this.object.timePreempt / HitObject.preemptMin);
    ...
    private setDistances(clockRate: number) {
        ...
        if (
            ...
            if (this.mode === Modes.Droid) {
                ...
                const v =
                    this.object.stackedPosition.subtract(lastCursorPosition);
