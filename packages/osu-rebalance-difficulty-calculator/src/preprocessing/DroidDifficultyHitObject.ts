...
export class DroidDifficultyHitObject extends DifficultyHitObject {
    /**
     * Touch-specific data for this hit object.
     *
     * Populated by {@link DroidTouchActionSequenceOptimizer}.
     */
    touchData: DroidDifficultyHitObjectTouchData | null = null;
    ...
    override get overallDifficulty(): number {
        const { rawHitWindow } = this;
        const greatWindow = rawHitWindow.greatWindow / this.clockRate;

        if (rawHitWindow instanceof PreciseDroidHitWindow) {
            return PreciseDroidHitWindow.greatWindowToOD(greatWindow);
        } else {
            return DroidHitWindow.greatWindowToOD(greatWindow);
        }
    }
    ...
    override get smallCircleBonus(): number {
        return 1 + Math.pow(Math.max(0, 65 - this.object.radius) / 40, 2);
    }
